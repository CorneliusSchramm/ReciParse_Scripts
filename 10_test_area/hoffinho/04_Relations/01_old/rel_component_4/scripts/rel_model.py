...
from thinc.types import Floats3d, Ints1d, Ragged, cast
from thinc.api import Model, Linear, chain, Logistic


@spacy.registry.architectures.register("rel_model.v1")
def create_relation_model(
    create_instance_tensor: Model[List[Doc], Floats3d],
    classification_layer: Model[Floats3d, Floats3d],
) -> Model[List[Doc], Floats3d]:
...
def create_tensors(
    tok2vec: Model[List[Doc], List[Floats3d]],
    pooling: Model[Ragged, Floats3d],
...
def instance_forward(model: Model[List[Doc], Floats3d], docs: List[Doc], is_train: bool) -> Tuple[Floats3d, Callable]:
    ...
    def backprop(d_relations: Floats3d) -> List[Doc]:
...
def instance_init(model: Model, X: List[Doc] = None, Y: Floats3d = None) -> Model:
    tok2vec = model.get_ref("tok2vec")
    if X is not None:
