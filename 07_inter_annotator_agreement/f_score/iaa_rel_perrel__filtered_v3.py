...
"""{"head":4,"child":0,"head_span":{"start":29,"end":36,"token_start":4,"token_end":4,"label":"V"},"child_span":
{"start":0,"end":9,"token_start":0,"token_end":0,"label":"Z"},"color":"#c5bdf4","label":"ARG0"}"""

def create_ent_set(span_list, rel_list, rels_to_comp):
    """for each example recipe it creates a set of annotations (label, start, end)"""
    rel_set = set()

    span_information_set = set() #(token_start, token_end, entity)

    for entity in span_list:
        try: 
            span_information_set.add((entity["token_start"], entity["token_end"], entity["label"]))
    ...
    for entity in rel_list:
        try: 
            if entity["label"] in rels_to_comp:
                rel_set.add(( (entity["head_span"]["token_start"], entity["head_span"]["token_end"], entity["head_span"]["label"]),
                 (entity["child_span"]["token_start"], entity["child_span"]["token_end"], entity["child_span"]["label"]) , entity["label"]))   #start = start character, end = end character
    ...
    return {"entities": span_information_set, "relations": rel_set}
...
ov_dict_coco = {example["text"] : create_ent_set(example["spans"], example["relations"], rels_to_compare) for example in ov_recipes_coco}
ov_dict_graf = {example["text"] : create_ent_set(example["spans"], example["relations"], rels_to_compare) for example in ov_recipes_graf}
ov_dict_hoff = {example["text"] : create_ent_set(example["spans"], example["relations"], rels_to_compare) for example in ov_recipes_hoff}
ov_dict_jthn = {example["text"] : create_ent_set(example["spans"], example["relations"], rels_to_compare) for example in ov_recipes_jthn}
...
for i in range(len(ov_all_dicts)-1):
    ...
    for j in range(i+1, len(ov_all_dicts)):
        ...
        for key in ov_all_dicts[i].keys():      #loop through all recipes
            try:
                recipe_gold = ov_all_dicts[i][key]  #{"entities": span_information_set, "relations": rel_set}
                recipe_cand = ov_all_dicts[j][key]
                
                entity_intersection = recipe_gold["entities"].intersection(recipe_cand["entities"])

                filtered_rels_gold = {rel for rel in recipe_gold["relations"] if rel[0] in entity_intersection and rel[1] in entity_intersection}
                filtered_rels_cand = {rel for rel in recipe_cand["relations"] if rel[0] in entity_intersection and rel[1] in entity_intersection}

                tp, fp, fn = score_set(filtered_rels_gold, filtered_rels_cand)  #compare each recipe for both annotators
...
print("")
print(f"####   Individual filtered {rels_to_compare} comparison   ####")
